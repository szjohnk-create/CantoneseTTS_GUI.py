...
class CantoneseTTSApp:
    ...
    def open_settings(self):
        ...
        volume_frame.grid(row=1, column=1, sticky=tk.W, pady=10)
        ...
        speed_frame.grid(row=2, column=1, sticky=tk.W, pady=10)
        ...
        save_check.grid(row=3, column=0, columnspan=2, sticky=tk.W, pady=20)
        ...
        return_btn.grid(row=4, column=0, columnspan=2, pady=20)
    ...
    def get_voice_info(self):
        ...
        # 將 0-100 的語速轉換為 edge-tts 格式 (-50% 到 +100%)
        # 0 → -50% (很慢), 50 → 0% (正常), 100 → +100% (很快)
        speed_value = self.settings["speed"]
        if speed_value < 50:
            rate = f"-{50 - speed_value}%"
        elif speed_value == 50:
            rate = "+0%"
        else:
            rate = f"+{speed_value - 50}%"
    ...
    def get_output_filename(self):
        ...
        filename = f"{timestamp}_talk.mp3"
        output_dir = r"D:\PythonFiles\Output\TTS"
        os.makedirs(output_dir, exist_ok=True)
        return os.path.join(output_dir, filename)
    
    async def text_to_speech_async(self, text, output_file):
        """
        非同步文字轉語音
        """
        voice, rate = self.get_voice_info()
    ...
    def _read_text_thread(self, text):
        ...
        try:
            ...
            if self.settings["save_file"]:
                ...
                output_file = os.path.join(temp_dir, f"tts_temp_{datetime.now().strftime('%Y%m%d_%H%M%S')}.mp3")
            
            self.current_audio_file = output_file
            
            # 執行 TTS
            asyncio.run(self.text_to_speech_async(text, output_file))
